...
do {
    ...
    [void]$config.Add([PSCustomObject]@{
        ...
        $config.name | Get-Unique | ForEach-Object {
            $name = $_
            $config | Where-Object {$_.name -like $name} | ForEach-Object {$NewConfig.Add($_)}
        }

        $config.mainmenu | ForEach-Object {if ($_ -notin $NewConfig){$NewConfig.Add($_)}}
