...
"@
	...
	$PSSysTray = (Join-Path $ConfigPath -ChildPath \PSSysTray.ps1)
	if (test-path $PSSysTray) {
		remove-ite
	}
			Set-Content -Value $string -Path (Join-Path $ConfigPath -ChildPath \PSSysTray.ps1) -Force
			 | Get-Item
