            ...
            if ($MyScriptPath -and !$ExitOnly) {
                ...
                if ($method -eq 'PSFile') {
                    $MenuItem.Add_Click( {
                            ...
                            if ($AsAdmin) {Start-Process -FilePath 'C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe' -ArgumentList "-NoProfile -NoLogo -WindowStyle Hidden -ExecutionPolicy Bypass -File `"$MyScriptPath`"" -Verb RunAs -ErrorAction Stop}
