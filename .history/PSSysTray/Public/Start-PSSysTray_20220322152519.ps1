...
'
            ...
            if ($MyScriptPath -and !$ExitOnly) {
                ...
                if ($method -eq 'PSCommand') {
                    $MenuItem.Add_Click( {
                            ShowConsole
                            $MyScriptPath = $This.MyScriptPath #Used to find proper path during click event
                            if ($AsAdmin.ToLower() -like 'yes') {Start-Process -FilePath 'C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe' -ArgumentList "-NoProfile -NoLogo -ExecutionPolicy Bypass -Command `"& {$MyScriptPath}""" -Verb RunAs -ErrorAction Stop}
                ...
                if ($method -eq 'Other') {
                    $MenuItem.Add_Click( {
                            ShowConsole
                            $MyScriptPath = $This.MyScriptPath #Used to find proper path during click event
                            if ($AsAdmin.ToLower() -like 'yes') {Start-Process $MyScriptPath -Verb RunAs}
        ...
        foreach ($main in ($config.mainmenu | Get-Unique)) {
            ...
            $config | Where-Object { $_.Mainmenu -like $main } | ForEach-Object { NMenuItem -Text $_.ScriptName -MyScriptPath $_.ScriptPath -method $_.mode -AsAdmin $_.AsAdmin -MainMenu $tmpmenu }
        }
        $Menu_Exit = New-Object System.Windows.Forms.MenuItem
        $Menu_Exit.Text = 'Exit'
        $Menu_Exit.add_Click( {
