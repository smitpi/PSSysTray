...
'
        ...
        Function Invoke-Action {
            ...
            if ( $options -contains 'Hidden' ) { $processArguments.WindowStyle = 'Hidden' }
            if ( $options -contains 'Normal' ) { $processArguments.WindowStyle = 'Normal' }
            if ( $options -contains 'Maximized' ) { $processArguments.WindowStyle = 'Maximized' }
        ...
        foreach ($main in ($config.mainmenu | Get-Unique)) {
            ...
            foreach ($rec in $record) {
                ...
                [scriptblock]$clickAction = [scriptblock]::Create( "Invoke-Action -control `$_ -name `"$($rec.Name)`" -command `"$($rec.command)`" -arguments `"$(($rec|Select-Object -ExpandProperty arguments -ErrorAction SilentlyContinue) -replace '"' , '`"`"')`" -mode $($rec.Mode) -options `"$($op)`"" )
