...
'
        ...
        foreach ($main in ($config.mainmenu | Get-Unique)) {
            ...
            foreach ($rec in $record) {
                ...
                [scriptblock]$clickAction = [scriptblock]::Create( "Invoke-Action -control `$_ -name `"$($rec.Name)`" -command `"$($rec.command)`" -arguments `"$(($rec|Select-Object -ExpandProperty arguments -ErrorAction SilentlyContinue) -replace '"' , '`"`"')`" -mode $($rec.Mode) -command `"$($rec.command)`" -command `"$($rec.command)`"" )
